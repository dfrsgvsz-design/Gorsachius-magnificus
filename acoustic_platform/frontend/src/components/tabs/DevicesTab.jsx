  ...
  return (
    <div className="space-y-6">
      <PageHero
        ...
        actions={(
          <>
            <button
              onClick={loadDevices}
              disabled={refreshing}
              className="touch-button flex items-center justify-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs text-gray-300 hover:bg-white/10"
            >
            ...
            <button
              ...
              className="touch-button flex items-center justify-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-500/20 px-3 py-2 text-xs text-emerald-300 hover:bg-emerald-500/30"
            >
        ...
        metrics={(
          <>
            <StatCard label={t('devicesPage.registeredDevices')} value={devices.length} icon={Cpu} color="teal" />
            <StatCard label={t('devicesPage.onlineNow')} value={onlineDevices.length} icon={Wifi} color="forest" />
            <StatCard label={t('devicesPage.offlineNow')} value={offlineDevices.length} icon={WifiOff} color="amber" />
            <StatCard label={t('devicesPage.mappedLocations')} value={mappedDevices.length} icon={MapPin} color="blue" />
          </>
        )}
        aside={(
          <>
        ...
        <section className="section-shell space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <input
              ...
              className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder-gray-500"
            />
            <input
              value={form.name}
            ...
            <button
              ...
              className="touch-button rounded-lg bg-emerald-500 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-600 disabled:opacity-50"
            >
          ...
          devices.map((device) => (
            <div key={device.device_id} className="surface-card flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-3">
                <div className={`h-2.5 w-2.5 rounded-full ${device.online ? 'bg-emerald-400' : 'bg-gray-600'}`} />
                <div>
              ...
              <div className="flex flex-wrap items-center gap-3 md:justify-end">
                ...
                <button
                  ...
                  className="touch-button rounded-lg border border-red-500/20 px-2.5 text-red-400/60 transition-colors hover:bg-red-500/10 hover:text-red-400 disabled:opacity-40"
...
function DeviceMap({ devices, typeLabels, onlineLabel, offlineLabel, loadingLabel }) {
  ...
  return (
    <div id="device-map" className="h-72 overflow-hidden rounded-lg border border-white/10" style={{ background: '#1a1a2e' }}>
      {!mapReady && (
