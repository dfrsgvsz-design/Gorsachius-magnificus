...
const COPY = {
  ...
  zh: {
    ...
    packageStatusBody: '当前 package 元数据仍显示为 seed-only 或非 exhaustive，正式上线前仍需导入权威物种名录。',
    packageStatusReadyBody: '当前切片的 package 元数据已标记为 exhaustive。',
    packageSection: 'Taxonomy 名录包',
    ...
    protocolsLabel: '协议',
    catalogStatus: '名录状态',
    localSeedAssets: '本地 seed 资源',
    noPackages: '当前筛选条件下还没有可用的 taxonomy package 元数据。',
...
export default function SpeciesTab() {
  ...
  const allSeedOnly = packages.length > 0 && packages.every(
    (item) => item.seed_only || !item.exhaustive_species_content,
  )
  ...
  return (
    <div className="space-y-5">
      <section className="glass-card space-y-3 p-4 md:space-y-4 md:p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 max-w-3xl">
            <div className="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2.5 py-0.5 text-[11px] font-medium text-cyan-300 md:gap-2 md:px-3 md:py-1 md:text-xs">
          ...
          <button
            ...
            disabled={refreshing}
            className="touch-button inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1.5 text-[11px] text-gray-300 active:scale-[0.97] md:px-3 md:text-xs"
          >
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <label className="space-y-1.5">
            <span className="text-xs font-medium uppercase tracking-[0.2em] text-gray-400">{copy.module}</span>
            <div className="relative">
              <SelectedProgramIcon className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-emerald-300" />
              <select
                value={program}
                onChange={(event) => setProgram(event.target.value)}
                className="touch-button w-full rounded-xl border border-white/20 bg-white/10 py-2.5 pl-10 pr-4 text-sm text-white focus:border-emerald-500/50 focus:outline-none"
              >
          ...
          <label className="space-y-1.5">
            <span className="text-xs font-medium uppercase tracking-[0.2em] text-gray-400">{copy.jurisdiction}</span>
            <div className="relative">
              <MapPinned className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-cyan-300" />
              <select
                value={jurisdiction}
        ...
        <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-gray-400 md:gap-3 md:text-xs">
          <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 md:px-3 md:py-1">
      ...
      {loading ? (
        ...
        <>
          <section className={`glass-card flex items-start gap-3 p-4 ${allSeedOnly ? 'border border-amber-500/20 bg-amber-500/5' : 'border border-emerald-500/20 bg-emerald-500/5'}`}>
            {allSeedOnly ? (
            ...
            <div>
              <p className={`text-sm font-semibold ${allSeedOnly ? 'text-amber-200' : 'text-emerald-200'}`}>
          ...
          <section className="space-y-3">
            ...
            {packages.length === 0 ? (
              <div className="glass-card p-4 text-sm text-gray-400">{copy.noPackages}</div>
            ) : (
          ...
          <section className="space-y-3">
            ...
            {protocols.length === 0 ? (
              <div className="glass-card p-4 text-sm text-gray-400">{copy.noProtocols}</div>
            ) : (
              <div className="grid gap-3 lg:grid-cols-2">
                {protocols.map((item) => (
...
function PackageCard({ item, copy }) {
  const seedOnly = item.seed_only || !item.exhaustive_species_content
    ...
    <article className="glass-card space-y-3 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-white md:text-base">{item.label || item.package_id}</p>
          <p className="mt-1 break-all text-xs text-gray-500">{item.package_id}</p>
        </div>
        <span className={`rounded-full border px-2.5 py-1 text-[11px] font-medium ${seedOnly ? 'border-amber-500/30 bg-amber-500/10 text-amber-200' : 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200'}`}>
          {statusLabel}
        </span>
      </div>
  ...
  return (
    <article className="glass-card space-y-3 p-4">
      <div>
        ...
        <p className="mt-1 break-all text-xs text-gray-500">{item.protocol}</p>
      </div>
      {item.description ? (
