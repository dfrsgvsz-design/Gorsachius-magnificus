...
if (nativeApiConfigError) {
  console.error(`[api] ${nativeApiConfigError}`);
}

api.interceptors.request.use((config) => {
  if (nativeApiConfigError) {
    return Promise.reject(new Error(nativeApiConfigError));
  }
  return config;
});
...
export async function getSurveyProjects(filters = {}) {
  const resp = await api.get("/surveys/projects", {
...
export async function createSurveyProject(data) {
  const resp = await api.post("/surveys/projects", data);
  return resp.data;
}

export async function getFieldSurveySites(projectId = "") {
  const resp = await api.get("/surveys/sites", {
    params: buildSurveyParams(projectId),
  });
  return resp.data;
}

export async function createFieldSurveySite(data) {
  const resp = await api.post("/surveys/sites", data);
  return resp.data;
}

export async function getSurveyRoutes(projectId = "", siteId = "") {
  const resp = await api.get("/surveys/routes", {
    params: buildSurveyParams(projectId, siteId),
  });
  return resp.data;
}

export async function createSurveyRoute(data) {
  const resp = await api.post("/surveys/routes", data);
  return resp.data;
}
...
export async function getSurveyObservations(projectId = "", siteId = "") {
  const resp = await api.get("/surveys/observations", {
    params: buildSurveyParams(projectId, siteId),
  });
  return resp.data;
}
...
export async function createSurveyObservation(data) {
  const resp = await api.post("/surveys/observations", data);
  return resp.data;
}

export async function getSurveyTracks(projectId = "", siteId = "") {
  const resp = await api.get("/surveys/tracks", {
    params: buildSurveyParams(projectId, siteId),
  });
  return resp.data;
}
...
export async function createSurveyTrack(data) {
  const resp = await api.post("/surveys/tracks", data);
  return resp.data;
}

export async function createOfflineMapPackage(data) {
  const resp = await api.post("/surveys/offline/map-packages", data);
  return resp.data;
}
...
export async function getSurveyDesignAssets(filters = {}) {
  const resp = await api.get("/surveys/design-assets", {
    params: buildSurveyParams(filters),
  });
  return resp.data;
}
...
export async function createSurveyDesignAsset(data) {
  const resp = await api.post("/surveys/design-assets", data);
  return resp.data;
}

export async function getSurveyEvents(filters = {}) {
  const resp = await api.get("/surveys/events", {
    params: buildSurveyParams(filters),
  });
  return resp.data;
}
...
export async function createSurveyEvent(data) {
  const resp = await api.post("/surveys/events", data);
  return resp.data;
}

export async function createSurveyExportJob(jurisdiction, data = {}) {
  const resp = await api.post(
    `/surveys/exports/${encodeURIComponent(jurisdiction)}`,
    data,
  );
  return resp.data;
}
...
export async function analyzeOccupancy(body) {
  const resp = await api.post("/occupancy/analyze", body);
  return resp.data;
}

export async function getOccupancyVerificationTargets(species) {
  const resp = await api.get(
    `/occupancy/verification-targets/${encodeURIComponent(species)}`,
  );
  return resp.data;
}

// Phenology
export async function getPhenologyOverview(year) {
  const resp = await api.get(`/phenology/overview/${year}`);
  return resp.data;
}

export async function getPhenology(species, year) {
  const resp = await api.get(
    `/phenology/${encodeURIComponent(species)}?year=${year}`,
  );
  return resp.data;
}

export async function getPhenologyTrend(species, years) {
  const resp = await api.get(
    `/phenology/${encodeURIComponent(species)}/trend?years=${years}`,
  );
  return resp.data;
}

// Few-shot detectors
export async function listFewShotDetectors() {
  const resp = await api.get("/fewshot/detectors");
  return resp.data;
}

export async function createFewShotDetector(name, species, files) {
  const formData = new FormData();
  files.forEach((f) => formData.append("files", f));
  const resp = await api.post("/fewshot/create-detector", formData, {
    headers: { "Content-Type": "multipart/form-data" },
    params: { name, species },
  });
  return resp.data;
...
export async function deleteFewShotDetector(id) {
  const resp = await api.delete(`/fewshot/detectors/${id}`);
  return resp.data;
}
