"""Rate limiting middleware for the Biodiversity Field Survey Platform API."""
...
def _rate_state():
    ...
    if not isinstance(rate_limits, OrderedDict):
        rate_limits = OrderedDict(rate_limits or {})
...
async def rate_limit_middleware(request, call_next):
    ...
    if state._rate_gc_counter >= _RATE_GC_INTERVAL:
        ...
        for k in stale:
            del rate_limits[k]
    while len(rate_limits) > _MAX_RATE_CLIENTS:
