...
# ──────────────────────────────────────────────
# Rate limiting middleware (extracted to middleware/rate_limiter.py)
# ──────────────────────────────────────────────
...
_AUTH_EXEMPT_PREFIXES = ("/api/map-tiles/", "/api/maps/tiles/")
...
# ──────────────────────────────────────────────
# Runtime contract & health helpers
# ──────────────────────────────────────────────
...
def _align_species_mapping_to_checkpoint(expected_num_species: Optional[int]):
    """Trim mapping metadata to the classes actually present in the checkpoint."""
    ...
    logger.warning(
        "Species mapping (%d) does not match checkpoint head (%d); trimming runtime mapping to checkpoint classes.",
        current_num_species,
...
# Pydantic helper
# ──────────────────────────────────────────────
def _model_to_dict(model_instance: BaseModel) -> dict:
...
def predict_species(
    ...
    for prob, idx in zip(top_probs.cpu().numpy(), top_indices.cpu().numpy()):
        ...
        reliable = bool(conf > 0.3 and normalized_entropy < 0.7)
        if is_ood:
            reliable = False
        results.append(
            {
...
# ──────────────────────────────────────────────
# Register routers from route modules
# ──────────────────────────────────────────────
...
async def _lifespan_startup():
    """Lifespan startup: load model, initialize all stores."""
