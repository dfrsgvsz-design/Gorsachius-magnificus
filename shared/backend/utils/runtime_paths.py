...
The desktop bundle ships backend assets as extra data under a separate
`backend/` directory. Frozen modules still report `__file__` inside the
PyInstaller extraction tree, so raw `Path(__file__).parent` lookups miss those
assets. These helpers keep development and packaged runs aligned.
