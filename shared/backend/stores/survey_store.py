...
try:
    ...
    from shared.backend.utils.runtime_paths import (
        get_backend_dir,
        get_data_dir,
        get_resource_data_dir,
    )

try:
    from taxonomy_catalog import get_taxonomy_catalog
except ImportError:  # pragma: no cover - shared package import path
    from shared.backend.stores.taxonomy_catalog import get_taxonomy_catalog
...
class SurveyStore:
    ...
    CREATE INDEX IF NOT EXISTS idx_survey_audit_log_ts ON survey_audit_log(timestamp);
    CREATE INDEX IF NOT EXISTS idx_survey_audit_log_entity ON survey_audit_log(entity_type, entity_id);
    """

    # Audit log retention: keep at most this many rows (newest win). Checked
    # opportunistically on insert so no separate scheduler is required.
    _AUDIT_MAX_ROWS = 50_000
    ...
    def append_audit_entry(self, entry: dict) -> None:
        """Insert one audit row; trims oldest rows past `_AUDIT_MAX_ROWS`."""

        def _append() -> None:
            self._conn.execute(
                "INSERT INTO survey_audit_log "
                "(timestamp, device_id, user_id, op, entity_type, entity_id,"
                " method, path, ip, status_code, request_id) "
                "VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                (
                    str(entry.get("timestamp") or _utc_now()),
                    str(entry.get("device_id") or ""),
                    str(entry.get("user_id") or ""),
                    str(entry.get("op") or "unknown"),
            ...
            row = self._conn.execute(
                "SELECT COUNT(*) AS n FROM survey_audit_log"
            ).fetchone()
            if row and int(row["n"]) > int(self._AUDIT_MAX_ROWS * 1.05):
                self._conn.execute(
                    "DELETE FROM survey_audit_log WHERE id NOT IN "
                    "(SELECT id FROM survey_audit_log ORDER BY id DESC LIMIT ?)",
                    (self._AUDIT_MAX_ROWS,),
                )

        self._run_with_retry(_append, write=True)
    ...
    def list_audit_entries(
        ...
        """Return newest audit entries first, optionally filtered."""
    ...
    def purge_expired_trash(
        ...
        Every purged row is first archived as JSON-lines under
        ``<store>/trash_archive/<YYYYMMDD>_<table>.jsonl`` (or `archive_dir`)
        so accidental GC can still be recovered from disk. Returns a summary
        ``{"purged": {entity_type: count}, "archived_files": [...], "cutoff": ...}``.
        """
        ...
        archive_root = (
            Path(archive_dir) if archive_dir else (self._dir / "trash_archive")
        )
        ...
        def _purge() -> dict:
            ...
            for entity_type, meta in self._ENTITY_META.items():
                table = meta["table"]
                try:
                    rows = self._conn.execute(
                        f"SELECT * FROM {table} "
                    ...
                    continue  # table without deleted_at column
                if not rows:
                    continue
                archive_root.mkdir(parents=True, exist_ok=True)
                archive_path = archive_root / f"{stamp}_{table}.jsonl"
                ...
                cur = self._conn.execute(
                    f"DELETE FROM {table} WHERE deleted_at!='' AND deleted_at<?",
                    (cutoff,),
                )
                purged[entity_type] = cur.rowcount
            return {
                "purged": purged,
                "purged_total": sum(purged.values()),
                "archived_files": sorted(set(archived_files)),
                "cutoff": cutoff,
                "retention_days": retention_days,
            }

        summary = self._run_with_retry(_purge, write=True)
        if summary["purged_total"]:
