...
async def get_phenology(species: str, year: int = Query(default=2025)):
    ...
    return _m.phenology_engine.compute_phenometrics(species, year)
...
async def get_phenology_trend(
    ...
    return _m.phenology_engine.detect_phenological_shift(species, year_list)
...
async def get_phenology_overview(year: int):
    ...
    return {"year": year, "species": _m.phenology_engine.get_species_overview(year)}
