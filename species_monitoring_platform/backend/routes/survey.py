...
@router.delete(
    "/api/surveys/{site_name}", dependencies=[Depends(verify_admin_token)]
)
...
@router.delete(
    "/api/surveys/projects/{project_id}",
    tags=["Field Survey"],
...
@router.delete(
    "/api/surveys/observations/{observation_id}",
    tags=["Field Survey"],
    dependencies=[Depends(verify_admin_token)],
)
...
async def list_sync_conflicts(limit: int = Query(default=100, ge=1, le=500)):
    ...
    conflicts = _m.survey_store.get_conflicts(limit=limit)
...
async def resolve_sync_conflict(conflict_id: str, req: ConflictResolveRequest):
    ...
    if not _m.survey_store:
        raise HTTPException(status_code=503, detail="Survey store unavailable")
    try:
        result = _m.survey_store.resolve_conflict(
            conflict_id,
...
@router.post(
    "/api/surveys/projects/{project_id}/restore",
    tags=["Field Survey"],
    dependencies=[Depends(verify_admin_token)],
)
...
@router.post(
    "/api/surveys/sites/{site_id}/restore",
    tags=["Field Survey"],
    dependencies=[Depends(verify_admin_token)],
)
...
@router.post(
    "/api/surveys/routes/{route_id}/restore",
    tags=["Field Survey"],
    dependencies=[Depends(verify_admin_token)],
)
...
@router.post(
    "/api/surveys/observations/{observation_id}/restore",
    tags=["Field Survey"],
    dependencies=[Depends(verify_admin_token)],
)
...
async def list_survey_audit_log(
    limit: int = Query(default=200, ge=1, le=2000),
    entity_type: str = Query(default=""),
    op: str = Query(default=""),
):
    """Admin-only view of the survey audit trail (who/what/when/where)."""
    import main as _m

    if not _m.survey_store:
        return {"total": 0, "entries": []}
    entries = _m.survey_store.list_audit_entries(
        limit=limit, entity_type=entity_type, op=op
    )
