...
// MUST stay in sync with `TILE_CACHE` in `public/service-worker.js`.
// The service worker reads from this exact bucket via `staleWhileRevalidateTile`,
// so writes here are picked up automatically when the user goes offline.
const SHARED_TILE_CACHE_NAME = "bird-tile-cache-v4";
...
/** `${entity_type}:${entity_id}` keys the server confirmed (applied or
 *  deleted) — callers use these to clear the durable sync outbox (B24). */
export function collectAppliedSyncKeys(syncJob) {
  return Array.from(buildAppliedKeySet(syncJob));
}

/** Keys the server rejected as conflicts in this sync job. */
export function collectConflictSyncKeys(syncJob) {
  return Array.from(buildConflictKeySet(syncJob));
}

/**
 * Collapse a combined operation list (in-memory syncQueue + durable outbox)
 * to one op per (entity_type, entity_id, operation), keeping the newest by
 * `queued_at`. The result is sorted oldest-first; the server re-sorts by its
 * own entity priority before applying, so this order is only cosmetic.
 */
export function dedupeSyncOperations(operations) {
  const byKey = new Map();
  for (const op of operations || []) {
    if (!op || !op.entity_id) continue;
    const resolvedType = resolveEntityType(op.entity_type);
    const key = `${resolvedType}:${op.entity_id}:${op.operation || "upsert"}`;
    const existing = byKey.get(key);
    if (
      !existing ||
      String(op.queued_at || "") >= String(existing.queued_at || "")
    ) {
      byKey.set(key, { ...op, entity_type: resolvedType });
    }
  }
...
/** Merge durable outbox rows into the in-memory queue (bootstrap + badge
 *  refresh path). Pure: returns a new state object. */
export function mergeOutboxIntoState(state, outboxOps) {
  const current = normalizeSurveyStateShape(state || emptySurveyState());
  if (!Array.isArray(outboxOps) || outboxOps.length === 0) return current;
  return normalizeSurveyStateShape({
    ...current,
    syncQueue: dedupeSyncOperations([
      ...(current.syncQueue || []),
      ...outboxOps,
    ]),
  });
...
export async function prefetchMapTiles({
  ...
  minZoom,
  maxZoom,
  // `cacheKey` is accepted for backwards compatibility but no longer changes the
  // bucket — all preloaded tiles must land in the same cache the service worker
  // reads from, otherwise offline reads miss them entirely.
  ...
  const cache = await caches.open(SHARED_TILE_CACHE_NAME);
  const isProxiedTile =
    typeof tileUrl === "string" &&
    (tileUrl.includes("/api/map-tiles/") ||
      tileUrl.includes("/api/maps/tiles/"));
  // Same-origin proxied tiles can use a normal CORS request so that the service
  // worker `staleWhileRevalidateTile` handler later finds a clean Response. For
  // cross-origin OSM endpoints we keep `no-cors` so the opaque response can still
  // be cached — but we will only persist tiles whose status is OK.
  const requestInit = isProxiedTile ? undefined : { mode: "no-cors" };
  let downloaded = 0;
  let failed = 0;
