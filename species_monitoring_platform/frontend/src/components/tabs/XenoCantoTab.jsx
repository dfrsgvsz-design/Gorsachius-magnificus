    ...
    <div className="space-y-5">
      <section className="glass-card space-y-4 p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <a
            ...
            className="inline-flex items-center gap-1 text-xs text-[#BF5AF2] hover:text-[#BF5AF2]/80"
          >
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            {keyStatus?.configured ? (
              <span className="inline-flex items-center gap-1 rounded-full bg-[#30D158]/15 px-2.5 py-1 text-xs text-[#30D158]">
                <CheckCircle2 className="h-3.5 w-3.5" />
                {t('xenoCantoPage.configured')}
                {keyStatus.key_preview ? ` (${keyStatus.key_preview})` : ''}
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 rounded-full bg-[#FF9F0A]/15 px-2.5 py-1 text-xs text-[#FF9F0A]">
                <AlertCircle className="h-3.5 w-3.5" />
                {t('xenoCantoPage.notConfigured')}
              </span>
            )}
          </div>
          <p className="mb-3 text-xs leading-5 text-white/25">
            {t('xenoCantoPage.keyHelp')}
          </p>
          <div className="mb-3 flex flex-wrap items-center gap-3 text-xs text-white/40">
            <span className="rounded-full border border-white/[0.06] bg-white/[0.04] px-3 py-1">{t('xenoCantoPage.lastSync', { time: lastUpdatedLabel })}</span>
            <button
              onClick={loadKeyStatus}
              disabled={refreshing}
              className="inline-flex items-center gap-1 rounded-full border border-white/[0.06] bg-white/[0.04] px-3 py-1 text-white/50 hover:bg-white/[0.08] disabled:opacity-50"
            >
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <input
              ...
              className="flex-1 rounded-[12px] border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-sm text-white placeholder-white/20 focus:border-[#BF5AF2]/40 focus:outline-none"
            />
            <button
              onClick={handleSaveKey}
              disabled={!keyInput.trim() || savingKey}
              className="rounded-[12px] bg-[#BF5AF2] px-4 py-2 text-xs font-medium text-white transition-all hover:bg-[#BF5AF2]/80 disabled:opacity-40"
            >
      ...
      <section className="glass-card p-6">
        <h3 className="mb-4 flex items-center gap-2 text-lg font-semibold text-white">
          <Search className="h-5 w-5 text-[#BF5AF2]" />
        ...
        <div className="flex flex-col gap-3 lg:flex-row">
          <input
            ...
            className="flex-1 rounded-[12px] border border-white/[0.06] bg-white/[0.04] px-4 py-2.5 text-sm text-white placeholder-white/20 focus:border-[#BF5AF2]/40 focus:outline-none"
          />
          <select
            value={country}
            onChange={(event) => setCountry(event.target.value)}
            className="rounded-[12px] border border-white/[0.06] bg-white/[0.04] px-3 py-2.5 text-sm lg:w-[140px]"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-[12px] bg-[#BF5AF2] px-5 py-2.5 text-sm font-medium transition-all hover:bg-[#BF5AF2]/80 disabled:opacity-40 lg:min-w-[120px]"
          >
        ...
        <section className="glass-card p-4">
          ...
          <div className="max-h-[500px] space-y-2 overflow-y-auto">
            {results.recordings?.map((recording, index) => (
              <div key={`${recording.file_url || recording.id || index}`} className="flex items-center gap-4 rounded-2xl bg-white/[0.03] p-3 transition-all hover:bg-white/[0.06]">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#BF5AF2]/15 text-xs font-bold text-[#BF5AF2]">
                  {recording.quality}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-white">
                    {recording.species}
                    {' '}
                    <span className="italic text-white/25">({recording.scientific_name})</span>
                  </p>
                ...
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-xs text-white/40">{recording.type}</span>
                  {recording.file_url && (
                    <a
                      href={recording.file_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-[#BF5AF2] hover:text-[#BF5AF2]/80"
                    >
      ...
      {results && !results.error && results.total_results === 0 && (
        <section className="glass-card p-4 text-center text-sm text-white/40">
