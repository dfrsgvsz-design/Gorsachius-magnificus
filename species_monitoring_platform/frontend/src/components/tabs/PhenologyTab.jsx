...
const api = {
  getPhenology: (species, year) =>
    fetch(`/api/phenology/${encodeURIComponent(species)}?year=${year}`).then((r) => r.json()),
  getPhenologyTrend: (species, years) =>
    fetch(`/api/phenology/${encodeURIComponent(species)}/trend?years=${years}`).then((r) => r.json()),
  getPhenologyOverview: (year) =>
    fetch(`/api/phenology/overview/${year}`).then((r) => r.json()),
}
...
export default function PhenologyTab() {
  ...
  const loadOverview = useCallback(async () => {
    ...
    setError(null)
    try {
      const data = await api.getPhenologyOverview(year)
      setOverview(data)
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      api.getPhenology(selectedSpecies, year),
      api.getPhenologyTrend(selectedSpecies, `${year - 2},${year - 1},${year}`),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="rounded-[12px] border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-sm text-white">
          {[...Array(5)].map((_, i) => { const y = new Date().getFullYear() - i; return <option key={y} value={y}>{y}</option> })}
        </select>
        <select value={selectedSpecies || ''} onChange={(e) => setSelectedSpecies(e.target.value)} className="rounded-[12px] border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-sm text-white">
      ...
      {speciesList.length === 0 ? (
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          <section className="section-shell">
            ...
            ) : <p className="py-8 text-center text-sm text-white/25">{t('phenologyPage.selectSpecies', { defaultValue: 'Select a species to view its seasonal pattern' })}</p>}
          </section>
          ...
          <section className="section-shell">
            ...
            ) : <p className="py-8 text-center text-sm text-white/25">—</p>}
          </section>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="flex items-center gap-2 text-sm text-white/40">
                    {trend.first_detection_trend_days_per_year < 0 ? <TrendingDown className="h-4 w-4 text-[#FF9F0A]" /> : <TrendingUp className="h-4 w-4 text-[#0A84FF]" />}
                  ...
                  <p className="mt-1 text-xs text-white/25">p = {trend.first_detection_p_value}</p>
                </div>
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <p className="text-sm text-white/40">{t('phenologyPage.peakTrend', { defaultValue: 'Peak trend' })}</p>
                  ...
                  <p className="mt-1 text-xs text-white/25">p = {trend.peak_p_value}</p>
                </div>
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <p className="text-sm text-white/40">{t('phenologyPage.interpretation', { defaultValue: 'Interpretation' })}</p>
                  <p className={`mt-2 text-lg font-semibold ${trend.interpretation === 'advancing' ? 'text-[#FF9F0A]' : trend.interpretation === 'delaying' ? 'text-[#0A84FF]' : 'text-white/50'}`}>
