...
class DetectionStore:
    ...
    def batch_add(
        ...
        """Batch add detections from a session. Returns list of detection IDs."""
        ids = []
        ...
        with self._lock:
            for det in detections:
                ...
                record = {
                    ...
                    "site_name": site_name,
                    "time_offset": round(det.get("time_offset", 0.0), 2),
                    "timestamp": now,
                    "model_version": "",
    ...
    def save(self):
        """Explicit commit to ensure all pending writes are flushed to disk."""
        with self._lock:
            self._conn.commit()
